...
class GoodsCategory extends BaseService implements IGoodsCategory 
{
    ...
    */
    ...
    public function getGoodsCategoryBlock($shop_id)
    {
        ...
        $goods_category_block_query = $goods_category_block->getQuery(["shop_id"=>$shop_id], "*", "create_time desc");
    ...
    public function setGoodsCategoryBlock($id, $shop_id, $data)
    {
        // TODO Auto-generated method stub
        $goods_category_block = new NsGoodsCategoryBlockModel();
        $result = $goods_category_block->save($data, ["shop_id"=>$shop_id, "id"=>$id]);
        return $result;
    }
    /* (non-PHPdoc)
    ...
    public function addGoodsCategoryBlock($category_id, $shop_id=0)
    {
        // TODO Auto-generated method stub
        $goods_category = new NsGoodsCategoryModel();
        $goods_category_info = $goods_category->getInfo(["category_id"=>$category_id], "*");
        if(!empty($goods_category_info)){
            
            $goods_category_block = new NsGoodsCategoryBlockModel();
            $goods_category_block_info = $goods_category_block->getInfo(["category_id"=>$category_id], "*");
    ...
    public function getGoodsCategoryBlockList($shop_id)
    {
        ...
        $goods_brand = new NsGoodsBrandModel();
        $goods_category = new NsGoodsCategoryModel();
        $goods_category_block_list = $goods_category_block->getQuery(["shop_id"=>$shop_id,"is_show"=>1], "*", "sort asc");
        foreach($goods_category_block_list as $k=>$v){
            $goods_list = $goods->getGoodsViewList(1, 10, ["ng.category_id_1"=>$v["category_id"],"ng.state"=>1], "sort asc");
            $goods_category_block_list[$k]["goods_list"] = $goods_list["data"];
            //是否显示品牌
            if($v["is_show_brand"] == 1){
                $goods_brnd_list = $goods_brand->pageQuery(1, 8, ["category_id_1"=>$v["category_id"]], "sort asc", "*");
                $goods_category_block_list[$k]["brand_list"] = $goods_brnd_list["data"];
            }
            //是否显示二级分类
            if($v["is_show_lower_category"]){
                $second_category_list = $goods_category->getQuery(["pid"=>$v["category_id"]], "*", "sort asc");
                if(!empty($second_category_list)){
                    foreach($second_category_list as $t=>$m){
                        $goods_list = $goods->getGoodsViewList(1, 10, ["ng.category_id_2"=>$m["category_id"]], "sort asc");
     ...
     */
    public function getGoodsCategoryBlockQuery($shop_id)
    {
        // TODO Auto-generated method stub
        $goods_category = new NsGoodsCategoryModel();
        $goods = new NsGoodsViewModel();
        $goods_category_list = $goods_category->getQuery(["pid"=>0], "*", "sort asc");
        foreach($goods_category_list as $k=>$v){
            $goods_list = $goods->getGoodsViewList(1, 10, ["ng.category_id_1"=>$v["category_id"],"ng.state"=>1], "sort asc");
            $goods_category_list[$k]["goods_list"] = $goods_list["data"];
        }
    ...
    public function deleteGoodsCategoryBlock($category_id)
    {
        // TODO Auto-generated method stub
        $goods_category_block = new NsGoodsCategoryBlockModel();
        $retval = $goods_category_block->destroy(["category_id"=>$category_id]);
        return $retval;
    }
