...
class Shop extends BaseService implements IShop
{
    ...
    public function getShopUserConsume($shop_id, $uid)
    {
        ...
        $pay_money = $order->where(array(
            "shop_id" => $shop_id,
            "buyer_id" => $uid,
            "order_status" => 4
        ))->sum("pay_money");
        $refund_money = $order->where(array(
            "shop_id" => $shop_id,
            "buyer_id" => $uid,
            "order_status" => 4
        ))->sum("refund_money");
        $money = $pay_money - $refund_money;
        return $money;
    }
    ...
    public function deleteShopBankAccouht($condition)
    {
        // TODO Auto-generated method stub
        $shop_bank_account = new NsShopBankAccountModel();
        $retval = $shop_bank_account->where($condition)->delete();
        return $retval;
    }
