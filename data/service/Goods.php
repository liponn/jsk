...
class Goods extends BaseService implements IGoods
{
        ...
        try {
            ...
            if ($goods_id == 0) {
                ...
                if (! empty($sku_array)) {
                    ...
                    //修改时先删除原来的规格图片
                    $this->deleteGoodsSkuPicture(["goods_id"=>$goods_id]); 
                    //sku图片添加
                    $sku_picture_array = array();
                    if( !empty($sku_picture_values)){
                        $sku_picture_array = json_decode($sku_picture_values, true);
    ...
    public function ModifyGoodsOffline($condition)
    {
        ...
        if ($result > 0) {
            //商品下架成功钩子
            hook("goodsOfflineSuccess", ['goods_id' => $condition]);
            return SUCCESS;
        } else {
            return UPDATA_FAIL;
        }
    ...
    public function ModifyGoodsOnline($condition)
    {
        ...
        if ($result > 0) {
            //商品上架成功钩子
            hook("goodsOnlineSuccess", ['goods_id' => $condition]);
            return SUCCESS;
        } else {
            return UPDATA_FAIL;
        }
    ...
    public function deleteGoods($goods_id)
    {
        ...
        try {
            //商品删除之前钩子
            hook("goodsDeleteBefore", ['goods_id' => $goods_id]);
            // 将商品信息添加到商品回收库中
            ...
            if ($res > 0) {
                ...
                foreach ($goods_id_array as $k => $v) {
                    ...
                    //删除规格图片
                    $goods_sku_picture->destroy(['goods_id' => $v]);
                }
            }
            $this->goods->commit();
            if ($res > 0) {
                //商品删除成功钩子
                hook("goodsDeleteSuccess", ['goods_id' => $goods_id]);
                return SUCCESS;
            } else {
                return DELETE_FAIL;
            }
        ...
        try {
            ...
            foreach ($goods_id_array as $k => $v) {
                ...
                if (empty($goods_delete_obj)) {
                    ...
                    //商品的sku图片备份
                    $goods_sku_picture = new NsGoodsSkuPictureModel();
                    $goods_sku_picture_list = $goods_sku_picture->getQuery(['goods_id' => $v], "*", "");
    ...
    public function getGoodsDetail($goods_id)
    {
        ...
        $goos_sku_picture_query = $goos_sku_picture->getQuery(["goods_id"=>$goods_id], "*", '');
        ...
        foreach($goos_sku_picture_query as $k=>$v){
            if($v["sku_img_array"] != ""){
                ...
                $tmp_img_array = $album_picture->getQuery(["pic_id"=>["in",$v["sku_img_array"]]], "*", '');
    ...
    public function getGoodsSkuPicture($goods_id){
        ...
        $sku_picture_list=$goods_sku->getQuery(["goods_id"=>$goods_id], "*", "");
        foreach ($sku_picture_list as $k=>$v){
            ...
            if(!empty($sku_img_array)){
                ...
                foreach ($sku_img_str as $img_id){
                    ...
                    $picture_obj=$picture_model->getInfo(["pic_id"=>$img_id], "*");
    ...
    public function deleteAttributeService($attr_id)
    {
        ...
        hook("goodsAttributeDeleteSuccess", ['attr_id' => $attr_id]);
        return $res;
    }
    ...
    public function regainGoodsDeleted($goods_ids)
    {
        ...
        try {
            foreach ($goods_array as $goods_id) {
                ...
                //sku图片恢复
                $goods_sku_picture_delete = new NsGoodsSkuPictureDeleteModel();
                $goods_sku_picture_delete_list = $goods_sku_picture_delete->getQuery(['goods_id' => $goods_id], "*", "");
    ...
    public function copyGoodsInfo($goods_id)
    {
        ...
        //sku规格图片
        $goods_sku_picture = new NsGoodsSkuPictureModel();
        $goods_sku_picture_query = $goods_sku_picture->getQuery(["goods_id"=>$goods_id], "goods_id, shop_id, spec_id, spec_value_id, sku_img_array", '');
    ...
    public function getRandGoodsList(){
        $result = $this->goods->getQuery(['state'=>1], 'goods_id', '');
    ...
    public function getGoodsCoupon($goods_id,$uid){
        ...
        //通过商品id获取到优惠劵类型
        $coupon_goods_type_id_list = $coupon_goods->getQuery(['goods_id'=>$goods_id], 'coupon_type_id', '');
        //去除掉未开始的和已结束的
        foreach($coupon_goods_type_id_list as $k=>$v){
            $res = $coupon_type ->getInfo(['coupon_type_id'=>$v['coupon_type_id']],"start_time,end_time");
        ...
        foreach ($coupon_goods_type_id_list as $v){
            //已领取，已使用的数目
            $already_received = $coupon->getCount(['coupon_type_id'=>$v['coupon_type_id'],"state"=>['neq',0]]);
            $condition = array(
                'start_time' => array(
                    'ELT',
                    time()
            ...
            if(!empty($coupon_detial)){
                $receive_quantity = $coupon->getCount(["coupon_type_id"=>$coupon_detial['coupon_type_id'],"uid"=>$uid]);
