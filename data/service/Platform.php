...
//         $list = $this->setReturnList($queryList, $queryCount, $page_size);
    ...
    public function getRecommendGoodsQuery($shop_id)
    {
        ...
        foreach($group_list as $k=>$v){
            $goods_list = $goods->getGoodsViewList(1, 0, "FIND_IN_SET(".$v["group_id"].",ng.group_id_array) AND ng.state = 1", "ng.sort asc");
            //var_dump($goods_list);
...
//                     }
     ...
     */
    public function getRecommendGoodsList($shop_id)
    {
        // TODO Auto-generated method stub
        $group = new NsGoodsGroupModel();
        $goods = new NsGoodsViewModel();
        ...
        foreach($group_list as $k=>$v){
            $group_goods_list = array();
            $goods_list = $goods->getGoodsViewList(1, 4, "FIND_IN_SET(".$v["group_id"].",ng.group_id_array) AND ng.state = 1", "ng.sort asc");
            //var_dump($goods_list);
            if(!empty($goods_list["data"])){
                foreach($goods_list["data"] as $t=>$m){
                    $is_exist = true;
