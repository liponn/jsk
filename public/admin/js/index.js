...
function onloadMemberBalanceWithdrawCount(){
	$.ajax({
		type : 'post',
		url : __URL(admin_main + "/index/getMemberBalanceWithdrawCount"),
...
function onloadGoodsCount(){
	$.ajax({
		type : "post",
		url : __URL(admin_main+"/index/getgoodscount"),
		success : function(data) {
			if(data['all']!=null){
...
function onloadSalesStatistics(){
	$.ajax({
		type : "post",
		url : __URL(admin_main+"/index/getsalesstatistics"),
		success : function(data) {
			$(".yesterday_goods").html(data['yesterday_goods']);
			$(".month_goods").html(parseFloat(data['month_goods']).toFixed(2));
			$(".yesterday_money").html(parseInt(data['yesterday_money']));
			$(".month_money").html(parseFloat(data['month_money']).toFixed(2));
			$(".js-order-amount").html(parseFloat(data['year_money']).toFixed(2));//今年订单总金额
			exeryTimePut("js-month-sales",parseInt(data['month_goods']));//本月销量
		}
	});
}
...
function onloadOrderChart(e) {
	...
	$.ajax({
		type : "post",
		url : __URL(admin_main+"/index/getorderchartcount"),
		data : {'date':e},
		success : function(data) {
...
function onloadConsultCount(){
	$.ajax({
		type : "post",
		url : __URL(admin_main+"/index/getconsultcount"),
		success : function(data) {
...
function onloadWeiXinFansCount(){
	$.ajax({
		type : "post",
		url : __URL(admin_main + "/index/getWeiXinFansCount"),
...
function onloadWeiXinFansChart(e) {
	...
	$.ajax({
		type : "post",
		url : __URL(admin_main+"/index/getweixinfanschartcount"),
		data : {'date':e},
		async : true,
		success : function(data) {
			var timeOut = "关注人数";
