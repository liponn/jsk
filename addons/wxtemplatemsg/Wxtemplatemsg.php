...
class Wxtemplatemsg extends \addons\Addons
{
    ...
    public function orderCreateSuccess($params = []){
        ...
        $url = __URL__."/wap/order/orderdetail?orderId=$order_id";
        $keyword1 = $order_info['order_no'];            //订单编号
        $keyword2 = $order_info['create_time'];         //创建时间
    ...
    public function orderOnLinePaySuccess($params = []){
        ...
        foreach ($order_id_array as $k => $order_id) {
            ...
            $url = __URL__."/wap/order/orderdetail?orderId=$order_id";
            $keyword1 = $order_info['order_no'];            //订单编号
            $keyword2 = $order_info['pay_time'];                          //支付时间
    ...
    public function orderGoodsRefundAskforSuccess($params = []){
        ...
        foreach ($order_goods_id_arr as $k => $v){
            $goods_name = $goods_name. ',' .$order_goods->getInfo(['order_goods_id' => $v], 'goods_name')['goods_name'];
        }
    ...
    public function memberWithdrawApplyCreateSuccess($params = []){
        ...
        $info = $withdraw->getInfo(['id' => $id], '*');
        $uid = $info['uid'];
        $url = __URL__."/wap/member/balancewithdraw";
        $keyword1 = $info['cash'];                     //本次提现金额
        $keyword2 = $info['account_number'];                     //提现账户
        $keyword3 = $info['ask_for_date'];                     //申请时间
        $keyword4 = '';                     //预计到账时间
     ...
     * 会员注册成功通知
     */
    public function userRegisterSuccess($params = []){
        $uid = $params['uid'];
        $url = '';
        $keyword1 = $params['member_name'];                     //会员昵称
        $keyword2 = $params['reg_time'];                     //注册时间
        $keyword3 = '';                     
        $keyword4 = '';                     
    ...
    public function promoterApplyCreateSuccess($params = []){
        ...
        $keyword2 = $params['regidter_time'];                     //通过时间
        $keyword3 = '';                     
        $keyword4 = '';                     
    ...
    protected function getOpenidByUid($uid){
        $uesr = new UserModel();
        //获取会员的openid
        $openid = $uesr->getInfo(['uid' => $uid], 'wx_openid');
        if($openid){
...
SQL;
        ...
        if(count(db()->query("SHOW TABLES LIKE '{$table_name}'")) != 1){
            ...
            $data = array(
                ['instance_id' => 0, 'template_no' => 'OPENTM203347141', 'template_id' => '', 'title' => '会员注册成功通知', 'is_enable' => 1, 'headtext' => '恭喜，您已成功注册为会员！', 'bottomtext' => '恭喜，您已成为会员，您将享受会员所有权利！'],
                ['instance_id' => 0, 'template_no' => 'OPENTM200444240', 'template_id' => '', 'title' => '订单提交成功通知', 'is_enable' => 1, 'headtext' => '亲！您已成功创建订单，点击进入完成付款。', 'bottomtext' => '感谢您的支持，我们将为您提供更好的服务。'],
                ['instance_id' => 0, 'template_no' => 'OPENTM201541214', 'template_id' => '', 'title' => '订单发货通知', 'is_enable' => 1, 'headtext' => '亲，你的订单已发货', 'bottomtext' => '请关注订单,注意查收'],
                ['instance_id' => 0, 'template_no' => 'OPENTM200444326', 'template_id' => '', 'title' => '订单付款成功通知', 'is_enable' => 1, 'headtext' => '亲！您的订单已成功付款。', 'bottomtext' => ''],
                ['instance_id' => 0, 'template_no' => 'OPENTM207103254', 'template_id' => '', 'title' => '退款申请', 'is_enable' => 1, 'headtext' => '您已申请退款，我们将尽快处理您提交的退款申请。您可以在“退换货”中查看退款进度', 'bottomtext' => '如您的退款有疑问，请联系我们的客服人员，帮助您解决处理！'],
                ['instance_id' => 0, 'template_no' => 'OPENTM205986235', 'template_id' => '', 'title' => '退款结果通知', 'is_enable' => 1, 'headtext' => '亲，您的退款已处理。', 'bottomtext' => '如您还有疑问，请与客服人员联系。'],
                ['instance_id' => 0, 'template_no' => 'OPENTM207292959', 'template_id' => '', 'title' => '提现申请提醒', 'is_enable' => 1, 'headtext' => '亲，您的提现申请已提交', 'bottomtext' => '我们将在1到3个工作日内处理完毕，请耐心等待。'],
                ['instance_id' => 0, 'template_no' => 'OPENTM207292959', 'template_id' => '', 'title' => '提现审核结果通知', 'is_enable' => 1, 'headtext' => '亲，您提现申请已通过', 'bottomtext' => '我们将在1到3个工作日内处理完毕，请耐心等待。'],
                ['instance_id' => 0, 'template_no' => 'OPENTM409846856', 'template_id' => '', 'title' => '申请通过通知', 'is_enable' => 1, 'headtext' => '您的推广员申请已经通过', 'bottomtext' => '如您还有疑问，请与客服人员联系。'],
                ['instance_id' => 0, 'template_no' => 'OPENTM409846856', 'template_id' => '', 'title' => '下级申请通过通知', 'is_enable' => 1, 'headtext' => '您的下级推广员申请已经通过', 'bottomtext' => '如您还有疑问，请与客服人员联系。'],
                ['instance_id' => 0, 'template_no' => 'OPENTM409846856', 'template_id' => '', 'title' => '下下级申请通过通知', 'is_enable' => 1, 'headtext' => '您的下下级推广员申请已经通过', 'bottomtext' => '如您还有疑问，请与客服人员联系。'],
                ['instance_id' => 0, 'template_no' => 'OPENTM201010537', 'template_id' => '', 'title' => '本店分销订单提成通知', 'is_enable' => 1, 'headtext' => '亲，您已成功分销出一笔订单，继续努力哦', 'bottomtext' => ''],
                ['instance_id' => 0, 'template_no' => 'OPENTM201010537', 'template_id' => '', 'title' => '下级分店分销订单提成通知', 'is_enable' => 1, 'headtext' => '亲，您的下级推广员成功分销出一笔订单，继续努力哦', 'bottomtext' => ''],
                ['instance_id' => 0, 'template_no' => 'OPENTM201010537', 'template_id' => '', 'title' => '下下级分店分销订单提成通知', 'is_enable' => 1, 'headtext' => '亲，您的下下级推广员成功分销出一笔订单，继续努力哦', 'bottomtext' => ''],
            );
